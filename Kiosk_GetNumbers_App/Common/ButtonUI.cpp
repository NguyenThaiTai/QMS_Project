...
void CButtonUI::DrawBackButton(Gdiplus::Graphics& g, int cx, int cy, Gdiplus::RectF& m_rectBack)
{
	float btnW = 180.0f;
	float btnH = 50.0f;
	m_rectBack = Gdiplus::RectF(cx - btnW / 2, cy, btnW, btnH);

	Gdiplus::GraphicsPath path;
	CButtonUI::AddRoundedRectToPath(path, m_rectBack, 25.0f);

	g.DrawPath(&Gdiplus::Pen(Gdiplus::Color(255, 162, 32, 45)), &path);
	...
	g.DrawString(L" \x2190  Quay lại", -1, &font, m_rectBack, &format, &Gdiplus::SolidBrush(Gdiplus::Color(255, 162, 32, 45)));
}