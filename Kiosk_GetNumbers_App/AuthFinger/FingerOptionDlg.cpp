#include "pch.h"
#include "FingerOptionDlg.h"

IMPLEMENT_DYNAMIC(FingerOptionDlg, CDialogEx)

BEGIN_MESSAGE_MAP(FingerOptionDlg, CDialogEx)
    ON_WM_PAINT()
    ON_WM_ERASEBKGND()
    ON_WM_LBUTTONUP()
    ON_WM_LBUTTONDOWN()
...
FingerOptionDlg::FingerOptionDlg(CWnd* pParent)
    : CDialogEx(IDD_FINGER_OPTIONS_DIALOG, pParent)
{
    m_bNewPressed = false;
    m_bExistPressed = false;
    m_pIconAddUser = nullptr;
    m_pIconFinger = nullptr;
}

FingerOptionDlg::~FingerOptionDlg()
{
    if (m_pIconAddUser) delete m_pIconAddUser;
    if (m_pIconFinger) delete m_pIconFinger;
}

BOOL FingerOptionDlg::OnInitDialog()
{
    CDialogEx::OnInitDialog();
    CHeaderUI::SetFullScreen(this);

    m_pIconAddUser = nullptr;
    m_pIconFinger = Common::LoadPNGFromResource(IDB_PNG_FINGER);

    SetTimer(1, 1000, NULL);
    return TRUE;
}

BOOL FingerOptionDlg::OnEraseBkgnd(CDC* pDC) { return TRUE; }

void FingerOptionDlg::OnPaint()
{
    CPaintDC dc(this);
    ...
    DrawOptionCard(g, m_rectOptNew, m_pIconAddUser, L"Chưa đăng ký", L"Đăng ký vân tay mới để nhận diện nhanh chóng", m_bNewPressed);
    DrawOptionCard(g, m_rectOptExist, m_pIconFinger, L"Đã đăng ký", L"Sử dụng vân tay đã đăng ký để xác thực", m_bExistPressed);
...
void FingerOptionDlg::DrawTitle(Gdiplus::Graphics& g, int cx, int cy)
{
    ...
    g.DrawString(L"Xác thực vân tay", -1, &fontTitle, Gdiplus::PointF(cx, cy - 350), &format, &Gdiplus::SolidBrush(Gdiplus::Color(255, 33, 37, 41)));
...
void FingerOptionDlg::DrawOptionCard(Gdiplus::Graphics& g, Gdiplus::RectF rect, Gdiplus::Image* pIcon, CString strTitle, CString strDesc, bool bIsPressed)
{
    ...
    if (pIcon) {
        ...
        g.DrawImage(pIcon, iconRect, 0, 0,
            (float)pIcon->GetWidth(), (float)pIcon->GetHeight(),
...
void FingerOptionDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void FingerOptionDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
    Gdiplus::PointF p((float)point.x, (float)point.y);
...
void FingerOptionDlg::OnTimer(UINT_PTR nIDEvent) {
    if (nIDEvent == 1) Invalidate(FALSE);
    CDialogEx::OnTimer(nIDEvent);
}

void FingerOptionDlg::DrawAddUserIcon(Gdiplus::Graphics& g, float x, float y, float size, Gdiplus::Color color)
{
    Gdiplus::SolidBrush brush(color);

    float personShiftX = size * 0.15f; 
    float personCenterY = y + size * 0.05f;
